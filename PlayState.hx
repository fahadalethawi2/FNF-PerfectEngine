...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['A', 0.9], //From 80% to 89%
		['S', 1], //From 90% to 99%
		['PLUS', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		add(songTxt);
		songTxt.text = curSong + " (" + storyDifficultyText + ") " + "| Perfect! Engine v" + MainMenuState.PerfectEngineVersion;
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Combo: ' + combo + ' | Score: ' + songScore + ' | Perfect Breaks: ' + songMisses + ' | Accuracy: ' + ratingName;
		} else {
			scoreTxt.text = 'Combo: ' + combo + ' | Score: ' + songScore + ' | Perfect Breaks: ' + songMisses + ' | Accuracy: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
		...
		{
			renderedTxt.text = 'Engine by fahadGX';
		}
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					if(ClientPrefs.timeBarType == 'Song Name + Time')
						timeTxt.text = SONG.song + ' (' + FlxStringUtil.formatTime(secondsTotal) + ')';
				}
			}
